...
function fixPostBody() {
    ...
    n ? cb_entryId > 92e5 && $.getScript("https://static-origin.cnblogs.com/scripts/mermaid/mermaid-8.0.0-rc8.min.js") : (cb_entryId > 5928739 && $("div.cnblogs_code").each(function () {
        var t, n;
...
function deliverAdC1() {
    $.ajax({
        url: location.protocol + "//a1.cnblogs.com/group/C1-T2",
        type: "get",
        dataType: "json",
        success: function (n) {
            n && ($("#ad_t2").html(n.T2), $("#cnblogs_c1").html(n.C1))
        }
    })
}
...
function deliverAdC2() {
    $.ajax({
        url: "https://promotion.aliyun.com/promotion/adv/getAliyunAdv.htm?s=1569688aec3640ec876b7b89637003bf&p=page_1470967088195&pin=pos_1470967088195",
        type: "get",
        dataType: "jsonp",
        jsonp: "cback",
        jsonpCallback: "jsonpCallbackA1",
        success: function (n) {
            if (n.success && n.data.length > 0) {
                var u = n.data[0].content.replace(/\'/g, '"'), t = JSON.parse(u),
                    i = t[Math.floor(Math.random() * t.length)],
                    f = $("<img />", {src: i.img, alt: "", width: 468, height: 60}),
                    r = $("<a />", {href: i.url, target: "_blank"});
                f.appendTo(r);
                r.appendTo($("#cnblogs_c2"))
            }
        }
    })
}
